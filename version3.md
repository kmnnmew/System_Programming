...
| v2와 차이 | Study Board 탭 유지 + 커뮤니티 게시판 탭 추가 (리스트형 · 추천/비추천 · 댓글) |
...
| C-06 | 추천 (👍) | 전체 게시글 추천, 중복 방지 (localStorage) | ✅ |
| C-07 | 비추천 (👎) | 전체 게시글 비추천, 중복 방지 (localStorage) | ✅ |
| C-08 | 조회수 | 게시글 열람 시 자동 증가 (Supabase UPDATE) | ✅ |
| C-09 | 이미지 첨부 | 파일 업로드 → base64 저장 (3MB 이하) | ✅ |
| C-10 | 동영상 첨부 | YouTube URL 입력 + 썸네일 미리보기 | ✅ |
| C-11 | 댓글 작성 | 게시글 댓글 입력 (Enter 키 단축키) + Supabase 저장 | ✅ |
| C-12 | 댓글 삭제 | 댓글 삭제 (확인창 후 DB 반영) | ✅ |
| C-13 | 추천순 정렬 | 추천 많은 순으로 정렬 | 🔜 추후 |
| C-14 | 검색 기능 | 제목·내용 키워드 검색 | 🔜 추후 |
| C-15 | 페이지네이션 | 게시글 목록 페이지 분할 | 🔜 추후 |
| C-16 | 로그인 연동 | Supabase Auth로 작성자 식별 | 🔜 추후 |
...
-- 샘플(더미) 게시글은 UUID 아이므로 댓글 미지원 (안내 메시지 표시)
```
...
[React 앱 - GitHub Pages]  ← /#v3 진입 시 AppV3 렌더링
       ...
       │
       └─ Supabase (PostgreSQL)
            ├─ board_posts  : 게시글 CRUD + 조회수/추천 UPDATE
